...
const tasks = {user:'',activity:[{ id: 0, text: "ciao" },{ id: 1, text: "ciao" }]};
let index = 2;
...
const [list,dispatch] = useImmerReducer(Reducer,tasks);
  ...
  return (
    <div>
      ...
      <Login user={(user)=>{console.log(user);dispatch({type:'user',user:user})}}/>
    </div>
  );
}
...
function Reducer(draft, action) {
  switch (action.type) {
    case 'added':draft.activity.push({
          id: action.id,
          text: action.text
        });
        break;

    case 'user': draft.user=action.user; break;
